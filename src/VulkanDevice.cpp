...
VulkanDevice::VulkanDevice(Window& window) : window(window), instance(VK_NULL_HANDLE) {
    createInstance();
}
...
void VulkanDevice::createInstance() {
    ...
    if (!checkExtensionSupport(requiredExtensions.data(), static_cast<uint32_t>(requiredExtensions.size()), extensions)) {
...
bool VulkanDevice::checkExtensionSupport(
    ...
    for (uint32_t i = 0; i < requiredCount; i++) {
        ...
        if (!found) {
            std::cerr << "Missing required extension: " << requiredExtensions[i] << std::endl;
            return false;
        }
    }
    return true;
